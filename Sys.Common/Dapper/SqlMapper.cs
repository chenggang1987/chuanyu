...
this IDbConnection cnn, string sql, Func<TFirst, TSecond, TThird, TFourth, TReturn> map, dynamic param = null, IDbTransaction transaction = null, bool buffered = true, string splitOn = "Id", int? commandTimeout = null, CommandType? commandType = null
        ...
        [Browsable(false), EditorBrowsable(EditorBrowsableState.Never)]
        [Obsolete("This method is for internal usage only", false)]
        public static DateTime? ReadNulllableDateTime(object value)
        {
            if (value == null || value is DBNull) return null;
...
#pragma warning disable 618
            ...
            if (type == typeof(DateTime))
            {
                return r => SqlMapper.ReadDateTime(r.GetValue(index));
            }
            if (type == typeof(DateTime?))
            {
                return r => SqlMapper.ReadNulllableDateTime(r.GetValue(index));
            }
...
)
        {
            ...
            foreach (var item in setters)
            {
                if (item.Property != null || item.Field != null)
                {
                    ...
                    {
                        il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod(
                            memberType == typeof(DateTime) ? "ReadDateTime" : "ReadNulllableDateTime"
...
dynamic param
        ...
        public void Add(
#if CSHARP30
            string name, object value, DbType? dbType, ParameterDirection? direction, int? size ,byet? scale
#else
string name, object value = null, DbType? dbType = null, ParameterDirection? direction = null, int? size = null, byte? scale = null
#endif
)
        {
            parameters[Clean(name)] = new ParamInfo() { Name = name, Value = value, ParameterDirection = direction ?? ParameterDirection.Input, DbType = dbType, Size = size, Scale = scale };
        }
